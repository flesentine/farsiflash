...
v5 is now the default learner curriculum. `index.html` loads the generated browser deck from `data/v5-deck.js`, which is exported from this canonical v5 pipeline.
...
The live browser deck is generated by `tools/export-v5-browser-deck.mjs` into `data/v5-deck.js`. The app now stores progress under `farsi2000-v5` using stable concept IDs.

The old `farsi2000-v4` localStorage record is never deleted by the v5 migration. Migration is intentionally conservative: only normalized Persian forms that match a v5 primary/spoken/formal form are transferred. The release audit currently measures **301/2000 (15.0%)** exact-form compatibility. Unmatched v4 progress is left in the untouched v4 state rather than guessed onto potentially different v5 concepts.

The release CI gate validates:
...
`deck.json` may still say `status: "foundation"` as provenance for the original build stage; the generated browser deck is the learner-facing v5 release.

**Steps 1–22 remain complete. Progress migration (#28) and make-v5-default (#31) are now implemented out of sequence by explicit release decision. Native Iranian review (#23) remains pending.**
