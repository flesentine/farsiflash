...
const romanized=applyRomanizationToCards(modernResult.cards,romanPolicy,registerPolicy);
...
if(cards.length!==2000) fail(`Romanization audit expects 2000 effective cards; found ${cards.length}`);
if(chunkResult.applied!==38) fail(`Romanization audit expects 38 Step 18 chunk promotions; found ${chunkResult.applied}`);
if(modernResult.applied!==11) fail(`Romanization audit expects 11 Step 21 modern-life promotions; found ${modernResult.applied}`);
...
for(const m of errors)console.error(`ERROR ${m}`);if(errors.length){console.error(`\nv5 Romanization audit failed: ${errors.length} error(s)`);process.exit(1);}console.log(`v5 Romanization audit passed: policy=${romanPolicy.version}, cards=${cards.length}, requiredPairs=${pairs.length}, files=${files.length}, chunkPromotions=${chunkResult.applied}, modernLifePromotions=${modernResult.applied}, examples=${cards.filter(c=>c.exampleRoman).length}`);
