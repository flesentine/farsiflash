...
const cards=applyRomanizationToCards(chunkResult.cards,romanPolicy,registerPolicy);
const errors=[]; const fail=m=>errors.push(m); const ids=new Map(cards.map((c,i)=>[c.id,i+1]));
const ROMAN_RE=/^[a-z0-9 /.,!?()]+$/;
if(cards.length!==2000) fail(`Romanization audit expects 2000 effective cards at Step 18; found ${cards.length}`);
...
for(const m of errors)console.error(`ERROR ${m}`);if(errors.length){console.error(`\nv5 Romanization audit failed: ${errors.length} error(s)`);process.exit(1);}console.log(`v5 Romanization audit passed: policy=${romanPolicy.version}, cards=${cards.length}, requiredPairs=${pairs.length}, files=${files.length}, chunkPromotions=${chunkResult.applied}`);
